...
export const split_rf_nodes = (nodes:Node[]):{
    group_nodes:Node[],
    default_nodes:Node[]
  } => {
    
    const used_node_ids:Set<string> = new Set();
    const group_nodes = nodes.filter((node) => node.type === "group");
    group_nodes.forEach((node) => {
      used_node_ids.add(node.id);
    });
   
    const default_nodes = nodes.filter((node) => !used_node_ids.has(node.id));
    return { group_nodes, default_nodes };
  }


export function sortByParent<T extends{id:string, parentId?:string}>(nodes:T[]):T[] {
    ...
    if (sorted.length !== nodes.length) {
      ...
      console.warn("Sorting mismatch: Not all nodes could be placed. Check for circular dependencies or missing parents.");
